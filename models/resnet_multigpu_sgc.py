...
from .utils import compute_gradcam_mask
...
class ResNet(nn.Module):
    ...
    def forward(self, images, contrastive_criterion, xe_images=None, aug_images=None, pred=None, targets=None,
        ...
        aug_gradcam_mask=aug_gradcam_mask*pred.squeeze(1)


        aug_gradcam_mask = normalize(aug_gradcam_mask.flatten(1))
        orig_aug_gradcam_mask = normalize(orig_gradcam_mask.flatten(1))
        ...
        if disable_contrast:
            contrastive_loss = torch.nn.functional.mse_loss(aug_gradcam_mask, orig_aug_gradcam_mask)
        else:
        ...
        return aug_images_outputs, xe_loss, contrastive_loss
